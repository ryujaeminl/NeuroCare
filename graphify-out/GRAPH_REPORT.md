...
## Corpus Check
- 269 files · ~170,552 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1542 nodes · 2561 edges · 120 communities (98 shown, 22 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 2 edges (avg confidence: 0.65)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d1778bf9`
...
- `WakeWordSettingsProps` --references--> `LinkedPatient`  [EXTRACTED]
  components/guardian/WakeWordSettings.tsx → hooks/useLinkedPatients.ts
...
Cohesion: 0.21
Nodes (14): buildSystemPrompt(), ChatMessage, PhotoToShow, POST(), ResponsesApiStreamEvent, SystemPromptResult, buildUpcomingFamilyPlans(), pickMessagePhotoToShow() (+6 more)

### Community 1 - "preferences/route.ts"
Cohesion: 0.11
Nodes (26): GET(), PATCH(), PreferencePatch, requireGuardian(), toWidgetOrder(), CHANNEL_LABELS, PreferenceState, SettingsPage() (+18 more)
...
Cohesion: 0.10
Nodes (5): ResponsesApiMessageItem, ResponsesApiOutputTextPart, ResponsesApiResult, { handlers, auth, signIn, signOut }, next-auth
...
Cohesion: 0.08
Nodes (34): POST(), POST(), MedicationForm(), MedicationFormProps, toDateInput(), isValidTimeString(), meetsAlertThreshold(), parseReminderTimes() (+26 more)
...
Cohesion: 0.15
Nodes (16): DELETE(), GET(), POST(), GET(), DELETE(), POST(), SubscribeInput, GET() (+8 more)

### Community 56 - "permissions.ts"
Cohesion: 0.16
Nodes (14): EmergencyInput, GET(), PATIENT_TRIGGER_TYPES, POST(), POST(), GET(), GET(), GET() (+6 more)

### Community 57 - "moodAnalysis.ts"
Cohesion: 0.28
Nodes (8): MOOD_VALUES, AnalyzableTurn, analyzeMood(), extractJson(), ResponsesApiMessageItem, ResponsesApiOutputTextPart, ResponsesApiResult, toMoodResult()

### Community 58 - "authErrorResponse"
Cohesion: 0.14
Nodes (24): GET(), PATCH(), DELETE(), PATCH(), FamilyMemberInput, GET(), POST(), DELETE() (+16 more)
...
Cohesion: 0.14
Nodes (20): formatDate(), isEnded(), isEndingSoon(), MedicationsPage(), GuardianPage(), FamilyMemberOption, PhotosPage(), DementiaStageSettingsProps (+12 more)
...
### Community 64 - ".continue/skills/caveman-compress/scripts/validate.py"
Cohesion: 0.15
Nodes (23): benchmark_pair(), count_tokens(), main(), print_table(), Path, count_bullets(), extract_code_blocks(), extract_headings() (+15 more)
...
### Community 83 - ".roo/skills/caveman-compress/scripts/validate.py"
Cohesion: 0.15
Nodes (23): benchmark_pair(), count_tokens(), main(), print_table(), Path, count_bullets(), extract_code_blocks(), extract_headings() (+15 more)

### Community 84 - "HistoryView.tsx"
Cohesion: 0.10
Nodes (27): hasConcerningStreak(), MoodSummaryCard(), MoodSummaryCardProps, SessionMood, formatDate(), formatTime(), HistorySessionSummary, HistoryTimeline() (+19 more)
...
### Community 89 - "requirePatientAccess"
Cohesion: 0.17
Nodes (14): FamilyMemberPatch, GET(), GET(), MemoryInput, GET(), PlanInput, POST(), validate() (+6 more)
...
Cohesion: 0.20
Nodes (10): generateInviteCode(), POST(), Session, DashboardLayout, DEMENTIA_STAGE_LABELS, DEMENTIA_STAGE_VALUES, DementiaStage, isUserRole() (+2 more)
...
Cohesion: 0.14
Nodes (19): DELETE(), MemoryPatch, PATCH(), POST(), POST(), serializeTags(), EMBEDDING_DIMENSION, EmbeddingPurpose (+11 more)
...
Cohesion: 0.28
Nodes (9): GET(), buildRecentCalendarEvents(), buildFamilyRoster(), buildPreviousSessionContext(), ROLE_LABEL, todayKeyKst(), buildRecentPlaysContext(), buildBasePersonaPrompt() (+1 more)
...
Cohesion: 0.27
Nodes (10): buildTodaysNewCalendarEventsContext(), formatDateLabel(), handleCalendarTurn(), takeTodaysNewCalendarEvents(), todayStartKst(), detectCalendarIntent(), ResponsesApiMessageItem, ResponsesApiOutputTextPart (+2 more)

### Community 108 - "강아지 마스코트 3D 스켈레톤 애니메이션 설계"
Cohesion: 0.22
Nodes (8): 강아지 마스코트 3D 스켈레톤 애니메이션 설계, 목표 / 범위, 배경, 범위 밖, 아키텍처, 에러 처리, 컴포넌트, 테스트 계획

### Community 109 - "dueMedicationContext.ts"
Cohesion: 0.36
Nodes (6): POST(), buildDueMedicationContext(), DUE_MEDICATION_WINDOW_MINUTES, DueMedication, findDueUnconfirmedMedication(), resolveConfirmationDateKey()
...
### Community 111 - "wakeGreeting.ts"
Cohesion: 0.60
Nodes (4): buildMorningGreetingContext(), DEFAULT_MORNING_WINDOW, kstMinutesOfDay(), todayDateKeyKst()

### Community 112 - "[patientId]/route.ts"
Cohesion: 0.67
Nodes (3): PATCH(), PatientPatch, isDementiaStage()

## Knowledge Gaps
- **518 isolated node(s):** `ResponsesApiStreamEvent`, `ChatMessage`, `PhotoToShow`, `SystemPromptResult`, `PATIENT_TRIGGER_TYPES` (+513 more)
...
- **Why does `prisma` connect `prisma.ts` to `chat/route.ts`, `preferences/route.ts`, `pineconeClient.ts`, `messages/route.ts`, `authOptions.ts`, `token/route.ts`, `emergencyDispatcher.ts`, `calendarEvents.ts`, `dueMedicationContext.ts`, `wakeGreeting.ts`, `[patientId]/route.ts`, `permissions.ts`, `requirePatientAccess`, `authErrorResponse`, `types.ts`?**
  _High betweenness centrality (0.011) - this node is a cross-community bridge._
- **Why does `authErrorResponse()` connect `authErrorResponse` to `preferences/route.ts`, `pineconeClient.ts`, `messages/route.ts`, `enroll/route.ts`, `[patientId]/route.ts`, `prisma.ts`, `permissions.ts`, `requirePatientAccess`?**
  _High betweenness centrality (0.009) - this node is a cross-community bridge._
- **Why does `WakeWordService` connect `WakeWordService` to `MainActivity`?**
...
- **Should `preferences/route.ts` be split into smaller, more focused modules?**
