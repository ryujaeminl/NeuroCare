...
동의했든, "다른 곡 틀어줘"처럼 재생 중 다른 곡으로 바꾸고 싶어하든) 그때
play_song을 호출하세요 - 이미 재생 중이어도 play_song만 다시 호출하면 자동으로
새 곡으로 바뀌니 stop_song을 먼저 부를 필요 없습니다. stop_song은 "그만"처럼
재생을 완전히 멈추라는 요청일 때만 호출하세요.`;
