...
- 무료 리깅된 glTF 강아지 모델(에셋 사이트에서 조달) 사용.
...
- **`public/models/dog.glb`** (신규 에셋): 무료 리깅 강아지 glTF. Idle/
  Listen/Think/Talk 4개 애니메이션 클립 포함(에셋에 없는 클립은 가장 가까운
  클립으로 대체 매핑 — 예: Talk 클립 없으면 Idle을 더 빠른 재생속도로 대용).
